...
func (s *HandlerSuite) TestSaveComment() {
	...
	url := fmt.Sprintf("/v1/api/alerts/%s/comments", dComment.Slug)
...
func (s *HandlerSuite) TestAlertComments() {
	// given
...
func (s *HandlerSuite) TestDeleteComment() {
	...
	// when
	url := fmt.Sprintf("/v1/api/alerts/%s/comments/%d", dComment.Slug, dComment.ID)
	res := httptest.NewRecorder()
